...
the published angle/distance trends. All results are printed as raw numbers, and the
two sweeps are plotted against the literature curves in ``voxel_rebound.pdf``.
...
# spray configuration (solver defaults unless noted; the injected mass is read back
# exactly via the solver's `record_generated_mass` flag)
...
class Example:
    ...
    def step(self):
        ...
        generated = self.solver.generated_droplet_mass.numpy().astype(np.float64)
        self.injected_mass += generated.sum(axis=1)

        # diagnostic: per-droplet (pre-rebound) mass that reached the wall region
        ...
        self.hit_mass += (near_wall * generated).sum(axis=1)
    ...
    def report(self):
        ...
        print(
            ...
            f"injected mass per event: {self.injected_mass.mean() / self.total_steps:.3f}"
        )
        print(
        ...
        self.save_plot(self.theta_deg[:n_angle], angle_r, dist_d, dist_r, formula, n_angle)
    ...
    def save_plot(
        ...
        """Write the two literature-overlay panels as a print-ready PDF."""
        try:
        ...
        from newton.examples.voxel import _plot_style  # noqa: PLC0415

        _plot_style.setup(plt)
        sim_color = _plot_style.SERIES[0]
        formula_color = "#86b6ef"
        melbye_color = _plot_style.SERIES[5]
        su_color = _plot_style.SERIES[1]

        fig, (ax_a, ax_d) = plt.subplots(1, 2, figsize=(7.2, 3.0))

        # angle sweep (fixed 1.0 m stand-off)
        ax_a.plot(angle_deg, angle_r, "o-", color=sim_color, markersize=3.0, label="simulation")
        ax_a.plot(angle_deg, formula[:n_angle], "--", color=formula_color, lw=1.1, label="solver rebound formula")
        in_range = MELBYE_ANGLE_DEG <= 80.0
        ax_a.plot(
            MELBYE_ANGLE_DEG[in_range],
            MELBYE_ANGLE_REBOUND[in_range],
            "--",
            color=melbye_color,
            lw=1.1,
            label="Melbye",
        )
        ax_a.set_xlabel("incidence angle from wall normal (deg)")
        ax_a.set_ylabel("rebound mass fraction")
        ax_a.set_title(f"angle sweep, {ANGLE_SWEEP_DISTANCE:.1f} m stand-off")
        ax_a.legend()

        # distance sweep (perpendicular)
        ax_d.plot(dist_d, dist_r, "o-", color=sim_color, markersize=3.0, label="simulation")
        ax_d.plot(dist_d, formula[n_angle:], "--", color=formula_color, lw=1.1, label="solver rebound formula")
        in_range = (MELBYE_DISTANCE_M >= dist_d.min()) & (MELBYE_DISTANCE_M <= dist_d.max())
        ax_d.plot(
            MELBYE_DISTANCE_M[in_range],
            MELBYE_DISTANCE_REBOUND[in_range],
            "--",
            color=melbye_color,
            lw=1.1,
            label="Melbye",
        ...
        ax_d.plot(
            SU_2022_DEM_DISTANCE_M,
            SU_2022_DEM_REBOUND,
            linestyle=(0, (3, 1.5, 1, 1.5)),
            color=su_color,
            lw=1.1,
            label="Su et al. (2022), DEM",
        )
        for i, (d_field, (lo, hi)) in enumerate(SU_2022_FIELD.items()):
            ax_d.errorbar(
                [d_field],
                [(lo + hi) / 2],
                yerr=[[(hi - lo) / 2], [(hi - lo) / 2]],
                fmt="s",
                markersize=3.0,
                color=su_color,
                capsize=2.5,
                elinewidth=0.9,
                label="Su et al. (2022), field" if i == 0 else None,
            )
        ax_d.set_xlabel("nozzle distance along spray axis (m)")
        ax_d.set_title("distance sweep, perpendicular")
        ax_d.legend()

        fig.suptitle("Rebound rate vs. literature", fontsize=9.5)
        fig.tight_layout()
        fig.savefig("voxel_rebound.pdf")
        plt.close(fig)
        print("saved figure: voxel_rebound.pdf")
